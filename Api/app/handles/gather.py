...
def receive_scanner_result():
    ...
    for transfer in result['client_ap_data_transfer']:
        ...
        exists = db.session.query(DataTransfer).filter_by(ap_mac=ap_mac, client_mac=client_mac, workspace=workspace).first()
    ...
    for auth in result['client_authorised']:
        ...
        exists = db.session.query(Auth).filter_by(ap_mac=ap_mac, client_mac=client_mac, workspace=workspace).first()
        if not exists:
